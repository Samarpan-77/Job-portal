...
<section class="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8 text-slate-900 shadow-soft lg:p-10">
    ...
    <div class="relative grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        ...
        <?php if ($image !== ''): ?>
            <img class="h-64 w-full rounded-3xl object-cover shadow-xl shadow-slate-100" src="<?= base_url($image) ?>" alt="Profile image">
        <?php else: ?>
...
<div class="mt-6 rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-soft backdrop-blur-sm">
    <?php if ($isEmployer): ?>
        <p class="text-slate-700"><strong class="text-slate-900">Company Name:</strong> <?= htmlspecialchars((string)($profile['company_name'] ?: $profile['name']), ENT_QUOTES, 'UTF-8') ?></p>
        <p class="mt-3 text-slate-700"><strong class="text-slate-900">Location:</strong> <?= htmlspecialchars((string)($profile['company_location'] ?: 'Not set'), ENT_QUOTES, 'UTF-8') ?></p>
        <p class="mt-3 text-slate-700"><strong class="text-slate-900">Website:</strong> <?= !empty($profile['company_website']) ? '<a class="font-semibold text-slate-700 hover:text-slate-900" href="' . htmlspecialchars((string)$profile['company_website'], ENT_QUOTES, 'UTF-8') . '" target="_blank" rel="noopener noreferrer">' . htmlspecialchars((string)$profile['company_website'], ENT_QUOTES, 'UTF-8') . '</a>' : 'Not set' ?></p>
        <p class="mt-3 text-slate-700"><strong class="text-slate-900">Contact:</strong> <?= htmlspecialchars((string)$profile['email'], ENT_QUOTES, 'UTF-8') ?></p>
    <?php else: ?>
        <p class="text-slate-700"><strong class="text-slate-900">Name:</strong> <?= htmlspecialchars((string)$profile['name'], ENT_QUOTES, 'UTF-8') ?></p>
        <p class="mt-3 text-slate-700"><strong class="text-slate-900">Headline:</strong> <?= htmlspecialchars((string)($profile['headline'] ?: 'Not set'), ENT_QUOTES, 'UTF-8') ?></p>
        <p class="mt-3 text-slate-700"><strong class="text-slate-900">Location:</strong> <?= htmlspecialchars((string)($profile['location'] ?: 'Not set'), ENT_QUOTES, 'UTF-8') ?></p>
        <p class="mt-3 text-slate-700"><strong class="text-slate-900">Website:</strong> <?= !empty($profile['website']) ? '<a class="font-semibold text-slate-700 hover:text-slate-900" href="' . htmlspecialchars((string)$profile['website'], ENT_QUOTES, 'UTF-8') . '" target="_blank" rel="noopener noreferrer">' . htmlspecialchars((string)$profile['website'], ENT_QUOTES, 'UTF-8') . '</a>' : 'Not set' ?></p>
    <?php endif; ?>

    <p class="mt-4 leading-8 text-slate-700"><strong class="text-slate-900">About:</strong> <?= nl2br(htmlspecialchars((string)($isEmployer ? ($profile['company_description'] ?: 'No description added yet.') : ($profile['bio'] ?: 'No bio added yet.')), ENT_QUOTES, 'UTF-8')) ?></p>
</div>

<div class="mt-6 flex flex-wrap gap-3">
    <a href="<?= base_url('profile/edit') ?>" class="inline-flex rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700">Edit Profile</a>
    <?php if ($isEmployer): ?>
    ...
    <?php endif; ?>
    <a href="<?= base_url('logout') ?>" class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100">Log Out</a>
</div>
