...
<section class="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8 text-slate-900 shadow-soft lg:p-10">
    ...
    <div class="relative grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div>
            ...
            <p class="mt-4 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base"><?= htmlspecialchars((string)($profile['headline'] ?: 'Professional profile'), ENT_QUOTES, 'UTF-8') ?></p>
        </div>
        <?php if (!empty($profile['profile_image'])): ?>
            <img class="h-64 w-full rounded-3xl object-cover shadow-xl shadow-slate-100" src="<?= base_url($profile['profile_image']) ?>" alt="<?= htmlspecialchars((string)$profile['name'], ENT_QUOTES, 'UTF-8') ?>">
        <?php else: ?>
...
<div class="mt-6 rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-soft backdrop-blur-sm">
    <p class="text-slate-700"><strong class="text-slate-900">Location:</strong> <?= htmlspecialchars((string)($profile['location'] ?: 'Not provided'), ENT_QUOTES, 'UTF-8') ?></p>
    ...
    <p class="mt-4 leading-8 text-slate-700"><strong class="text-slate-900">About:</strong> <?= nl2br(htmlspecialchars((string)($profile['bio'] ?: 'No bio added yet.'), ENT_QUOTES, 'UTF-8')) ?></p>
</div>
