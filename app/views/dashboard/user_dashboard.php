...
<section id="dashboardScroll" class="dashboard-scroll-container mx-auto max-w-7xl space-y-6">
    <section class="overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-950 text-white shadow-soft">
        <div class="bg-[radial-gradient(circle_at_top_right,_rgba(34,211,238,0.18),_transparent_30%),radial-gradient(circle_at_bottom_left,_rgba(14,165,233,0.16),_transparent_26%)] px-6 py-6 sm:px-8">
            <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-2xl">
                    <p class="text-xs font-semibold uppercase tracking-[0.25em] text-cyan-200">Home feed</p>
                ...
                <div class="grid grid-cols-3 gap-3 sm:min-w-[340px]">
                    <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Feed</p>
                        <p class="mt-2 text-2xl font-bold"><?= $feedCount ?></p>
                    </div>
                    <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Saved</p>
                        <p class="mt-2 text-2xl font-bold"><?= $savedJobCount ?></p>
                    </div>
                    <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Applied</p>
    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
                                ...
                                <div class="mt-5 flex flex-wrap items-center gap-3">
                                    ...
                                    <?php else: ?>
                                        <form method="POST" action="<?= base_url('job/save/' . $job['id']) ?>" class="inline-flex">
                                            <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                                            <input type="hidden" name="return_to" value="dashboard">
                                            <button class="inline-flex rounded-full border border-sky-200 bg-sky-50 px-4 py-2 text-sm font-semibold text-sky-700 transition hover:border-sky-300 hover:bg-sky-100">Save</button>
                                        </form>
                                    <?php endif; ?>
                                </div>
                            </div>
        ...
        <aside class="space-y-5 xl:sticky xl:top-24 xl:self-start">
            <div class="overflow-hidden rounded-[2rem] border border-slate-200 bg-white/90 shadow-soft backdrop-blur-sm">
                <div class="h-24 bg-gradient-to-r from-sky-600 via-cyan-500 to-teal-400"></div>
                <div class="px-6 pb-6">
                    <div class="-mt-10 flex items-end gap-4">
                        <?php if ($profileImage !== ''): ?>
                            <img
                                ...
                                class="h-20 w-20 rounded-3xl border-4 border-white object-cover shadow-lg shadow-sky-100">
                        <?php else: ?>
                            <div class="flex h-20 w-20 items-center justify-center rounded-3xl border-4 border-white bg-slate-100 text-2xl font-bold text-sky-700 shadow-lg shadow-sky-100">
                    ...
                    <div class="mt-5 flex flex-col gap-3 sm:flex-row xl:flex-col">
                        <a href="<?= base_url('profile') ?>" class="inline-flex justify-center rounded-full border border-sky-200 bg-sky-50 px-4 py-2 text-sm font-semibold text-sky-700 transition hover:border-sky-300 hover:bg-sky-100">View Profile</a>
            ...
            <div class="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-soft backdrop-blur-sm">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Suggested</p>
                        <h4 class="mt-2 text-xl font-bold tracking-tight text-slate-950">For you</h4>
                    </div>
                    <a href="<?= base_url('job/recommendations') ?>" class="text-sm font-semibold text-sky-700 hover:text-sky-600">Open</a>
                </div>
                ...
                <div class="mt-5 space-y-3">
                    ...
                    <?php else: ?>
                        <?php foreach ($recommendedJobs as $job): ?>
                            <a href="<?= base_url('job/view/' . $job['id']) ?>" class="block rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 transition hover:border-sky-200 hover:bg-sky-50">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        <h5 class="font-semibold text-slate-950"><?= htmlspecialchars((string)$job['title'], ENT_QUOTES, 'UTF-8') ?></h5>
                                        <p class="mt-1 text-sm text-slate-500">
                                            <?= htmlspecialchars((string)$job['employer_display_name'], ENT_QUOTES, 'UTF-8') ?>
                                        </p>
                                    </div>
                                    <span class="rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-sky-700 ring-1 ring-sky-200">
