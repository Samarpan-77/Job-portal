...
<section class="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8 text-slate-900 shadow-soft lg:p-10">
    ...
    <div class="relative grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div>
            ...
            <p class="mt-4 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base">Get high-level visibility into users, jobs, applications, and interview activity.</p>
        </div>
        <img class="h-64 w-full rounded-3xl object-contain bg-white/95 p-6 shadow-xl shadow-slate-100" src="<?= base_url('../images/logo.png') ?>" alt="Admin overview">
    </div>
</section>
...
<div class="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
    <div class="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-soft backdrop-blur-sm">
        ...
        <p class="mt-4 text-4xl font-bold tracking-tight text-slate-900"><?= $user_count ?></p>
    </div>

    <div class="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-soft backdrop-blur-sm">
        <h5 class="text-base font-semibold text-slate-600">Total Jobs</h5>
        <p class="mt-4 text-4xl font-bold tracking-tight text-slate-900"><?= $job_count ?></p>
    </div>

    <div class="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-soft backdrop-blur-sm">
        <h5 class="text-base font-semibold text-slate-600">Total Applications</h5>
        <p class="mt-4 text-4xl font-bold tracking-tight text-slate-900"><?= $application_count ?></p>
    </div>

    <div class="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-soft backdrop-blur-sm">
        <h5 class="text-base font-semibold text-slate-600">Interview Sessions</h5>
        <p class="mt-4 text-4xl font-bold tracking-tight text-slate-900"><?= $interview_count ?></p>
    </div>
</div>

<div class="mt-6 flex flex-wrap gap-3">
    <a href="<?= base_url('admin/users') ?>" class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100">Manage Users</a>
    <a href="<?= base_url('admin/applications') ?>" class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100">All Applications</a>
    <a href="<?= base_url('admin/interviewAnalytics') ?>" class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100">Interview Analytics</a>
</div>
...
<div class="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
    <div class="space-y-4">
        <div class="flex items-center justify-between gap-3">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Job feed</p>
                <h4 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">Latest jobs on the platform</h4>
            </div>
