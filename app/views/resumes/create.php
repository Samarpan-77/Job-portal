...
<section class="mx-auto max-w-5xl space-y-6">
    <div class="rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-soft backdrop-blur-sm">
        <h3 class="text-3xl font-bold tracking-tight text-slate-900">Create Resume</h3>
    ...
    <div class="rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-soft backdrop-blur-sm">
        <h5 class="text-lg font-bold tracking-tight text-slate-900">Choose Resume Template</h5>
        <div class="mt-5 grid gap-4 md:grid-cols-3 xl:grid-cols-5">
            <?php foreach ($templates as $templateId => $template): ?>
                <label class="template-card group cursor-pointer rounded-[1.5rem] border p-5 text-center transition <?= ($selectedTemplate === $templateId) ? 'border-slate-300 bg-slate-100 shadow-lg shadow-slate-100' : 'border-slate-200 bg-slate-50/80 hover:border-slate-300 hover:bg-slate-100' ?>">
                    ...
                    <h6 class="mt-3 text-sm font-semibold text-slate-900"><?= htmlspecialchars($template['name'], ENT_QUOTES, 'UTF-8') ?></h6>
    ...
    <form method="POST" action="<?= base_url('resume/store') ?>" class="rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-soft backdrop-blur-sm">
        ...
        <div class="space-y-5">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <input type="text" name="full_name" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" value="<?= htmlspecialchars($formData['full_name'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Professional Headline</label>
                <input type="text" name="headline" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" value="<?= htmlspecialchars($formData['headline'] ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="e.g. Full Stack Developer with 3+ years experience">
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Email</label>
                <input type="email" name="email" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" value="<?= htmlspecialchars($formData['email'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Phone</label>
                <input type="text" name="phone" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" value="<?= htmlspecialchars($formData['phone'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Address</label>
                <input type="text" name="address" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" value="<?= htmlspecialchars($formData['address'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Professional Summary</label>
                <textarea name="summary" class="min-h-32 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" rows="4" placeholder="Short profile summary"><?= htmlspecialchars($formData['summary'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Skills (comma separated)</label>
                <input type="text" name="skills" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" value="<?= htmlspecialchars($formData['skills'] ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="PHP, MySQL, JavaScript">
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Work Experience (one per line)</label>
                <textarea name="experience_items" class="min-h-32 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" rows="4" placeholder="Software Engineer - ABC Corp (2023-2025)&#10;Intern - XYZ Ltd (2022)"><?= htmlspecialchars($formData['experience_items'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Education (one per line)</label>
                <textarea name="education_items" class="min-h-32 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" rows="4" placeholder="BSc Computer Science - Tribhuvan University"><?= htmlspecialchars($formData['education_items'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Projects (one per line)</label>
                <textarea name="projects" class="min-h-24 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" rows="3" placeholder="Job Portal Web App - PHP, MySQL"><?= htmlspecialchars($formData['projects'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Certifications (one per line)</label>
                <textarea name="certifications" class="min-h-24 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-4 focus:ring-slate-100" rows="3" placeholder="AWS Cloud Practitioner"><?= htmlspecialchars($formData['certifications'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 flex flex-wrap gap-3">
            ...
            <button type="submit" name="action" value="save" class="inline-flex rounded-full bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-700">Save Resume</button>
        </div>
    </form>
...
<script>
    function selectTemplate(templateId) {
        ...
        document.querySelectorAll('.template-card').forEach(card => {
            card.classList.remove('border-slate-300', 'bg-slate-100', 'shadow-lg', 'shadow-slate-100');
        ...
        if (active && active.closest('.template-card')) {
            ...
            card.classList.add('border-slate-300', 'bg-slate-100', 'shadow-lg', 'shadow-slate-100');
