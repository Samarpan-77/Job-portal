...
<section class="space-y-6">
    <div class="rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-soft backdrop-blur-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h3 class="text-3xl font-bold tracking-tight text-slate-900"><?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?></h3>
            </div>
            <div class="flex flex-wrap gap-3">
                <?php if ($canDownload): ?>
                    <a href="<?= base_url('resume/downloadPDF/' . ($resumeId ?? 0)) ?>" class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100" title="Download as PDF">Download PDF</a>
                <?php endif; ?>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <?php foreach ($templates as $tId => $template): ?>
                            <form method="POST" action="<?= base_url('resume/changeTemplate/' . ($resumeId ?? 0)) ?>" class="inline-flex">
                                ...
                                <button type="submit" class="rounded-full border px-4 py-2 text-sm font-semibold transition <?= ($currentTemplateId === $tId) ? 'border-slate-300 bg-slate-100 text-slate-900' : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50' ?>" title="<?= htmlspecialchars($template['description'], ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="flex flex-wrap gap-3">
        ...
        <?php if (isset($readOnly) && !$readOnly): ?>
            <a href="<?= base_url('resume/edit/' . ($resumeId ?? 0)) ?>" class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100">Edit</a>
            <a href="<?= base_url('resume/delete/' . ($resumeId ?? 0)) ?>" class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100" onclick="return confirm('Are you sure?')">Delete</a>
        <?php endif; ?>
    </div>
</section>
