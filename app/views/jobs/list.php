...
<section class="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8 text-slate-900 shadow-soft lg:p-10">
    ...
    <div class="relative grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div>
            ...
            <p class="mt-4 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base">Explore current opportunities and open detailed job descriptions to apply with confidence.</p>
        </div>
        <img class="h-64 w-full rounded-3xl object-cover shadow-xl shadow-slate-100" src="<?= base_url('../images/Image2.png') ?>" alt="Career opportunities">
    </div>
</section>

<?php if ($searchQuery !== ''): ?>
    <div class="mt-6 flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
        <span>Showing results for <strong><?= htmlspecialchars($searchQuery, ENT_QUOTES, 'UTF-8') ?></strong></span>
        <a href="<?= base_url('job') ?>" class="inline-flex rounded-full border border-slate-200 bg-white px-3 py-1.5 font-semibold text-slate-700 transition hover:bg-slate-100">Clear search</a>
    </div>
<?php endif; ?>

<?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'employer'): ?>
    <a href="<?= base_url('job/create') ?>" class="mt-6 inline-flex rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700">Post New Job</a>
<?php endif; ?>
                        ...
                        <td class="px-6 py-5">
                            <?php if (!empty($job['image_path'])): ?>
                                <img
                                    ...
                                    class="h-16 w-28 rounded-2xl border border-slate-200 bg-white object-contain p-1">
                            <?php else: ?>
                                <span class="text-xs text-slate-400">No image</span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-5">
                            <div class="font-semibold text-slate-900"><?= htmlspecialchars($job['title']) ?></div>
                            <div class="mt-1 text-sm text-slate-500">
                                <a href="<?= base_url('profile/company/' . $job['employer_id']) ?>" class="font-medium text-slate-700 hover:text-slate-900">
                        ...
                        <td class="px-6 py-5">
                            <?php if (!empty($job['application_deadline'])): ?>
                                ...
                                <?php if ($job['application_deadline'] < date('Y-m-d')): ?>
                                    <div class="mt-1 text-xs font-semibold text-slate-500">Closed</div>
                                <?php endif; ?>
                            <?php else: ?>
                                ...
                                <?php if (($_SESSION['role'] ?? '') === 'user'): ?>
                                    ...
                                    <?php else: ?>
                                        <form method="POST" action="<?= base_url('job/save/' . $job['id']) ?>" class="inline-flex">
                                            <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                                            <input type="hidden" name="return_to" value="job">
                                            <button class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100">Save</button>
                                        </form>
                                    <?php endif; ?>
                                <?php endif; ?>
                                <?php
