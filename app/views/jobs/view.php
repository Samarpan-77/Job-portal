...
<section class="space-y-6">
    <div class="rounded-[2rem] border border-slate-200 bg-white/85 p-8 shadow-soft backdrop-blur-sm">
        <h3 class="text-3xl font-bold tracking-tight text-slate-950"><?= htmlspecialchars($job['title']) ?></h3>

        <?php if (!empty($job['image_path'])): ?>
            <div class="mb-6 overflow-hidden rounded-[2rem] border border-slate-200">
                <img
        ...
        <div class="grid gap-4 text-sm text-slate-700 sm:grid-cols-2">
            <p><strong class="text-slate-950">Location:</strong> <?= htmlspecialchars($job['location']) ?></p>
            <p><strong class="text-slate-950">Salary:</strong> <?= htmlspecialchars($job['salary']) ?></p>
            <p>
                <strong class="text-slate-950">Posted On:</strong>
            ...
            <p>
                <strong class="text-slate-950">Application Deadline:</strong>
            ...
            <p class="sm:col-span-2">
                <strong class="text-slate-950">Company:</strong>
                <a href="<?= base_url('profile/company/' . $job['employer_id']) ?>" class="font-semibold text-sky-700 hover:text-sky-600">
    ...
    <?php if ($canManageImage): ?>
        <div class="rounded-[2rem] border border-slate-200 bg-white/85 p-6 shadow-soft backdrop-blur-sm">
            <h5 class="text-lg font-bold tracking-tight text-slate-950">Manage Job Image</h5>
            <form method="POST" action="<?= base_url('job/uploadImage/' . $job['id']) ?>" enctype="multipart/form-data" class="mt-4 space-y-4">
                <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                <input type="file" name="image" class="block w-full rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-700 file:mr-4 file:rounded-full file:border-0 file:bg-sky-600 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white hover:file:bg-sky-500" accept=".jpg,.jpeg,.png,.webp,.gif,image/*" required>
                <button class="inline-flex rounded-full bg-sky-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-sky-500"><?= !empty($job['image_path']) ? 'Replace Image' : 'Upload Image' ?></button>
            </form>

            <?php if (!empty($job['image_path'])): ?>
                <form method="POST" action="<?= base_url('job/removeImage/' . $job['id']) ?>" class="mt-3">
                    <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                    <button class="inline-flex rounded-full border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-semibold text-rose-700 transition hover:border-rose-300 hover:bg-rose-100" onclick="return confirm('Remove this job image?');">Remove Image</button>
                </form>
            <?php endif; ?>
        </div>
    <?php endif; ?>
    ...
    <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'user'): ?>
        <div class="flex flex-wrap gap-3">
            ...
            <?php else: ?>
                <form method="POST" action="<?= base_url('job/save/' . $job['id']) ?>">
                    <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                    <input type="hidden" name="return_to" value="<?= 'job/view/' . $job['id'] ?>">
                    <button class="inline-flex rounded-full border border-sky-200 bg-sky-50 px-4 py-2 text-sm font-semibold text-sky-700 transition hover:border-sky-300 hover:bg-sky-100">Save Job</button>
                </form>
            <?php endif; ?>
        ...
        <?php if (empty($resumes)): ?>
            <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Create a resume first before applying.</div>
        ...
        <?php elseif ($isDeadlinePassed): ?>
            <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">This vacancy is closed and no longer accepting applications.</div>
        <?php else: ?>
            <form method="POST" action="<?= base_url('application/apply') ?>" class="space-y-4 rounded-[2rem] border border-slate-200 bg-slate-50/80 p-6">
                ...
                <select name="resume_id" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-sky-500 focus:ring-4 focus:ring-sky-100" required>
                ...
                <button class="inline-flex rounded-full bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-500">Apply</button>
            </form>
        <?php endif; ?>
    <?php endif; ?>
