...
<section class="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8 text-slate-900 shadow-soft lg:p-10">
    ...
    <div class="relative grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        ...
        <img class="h-64 w-full rounded-3xl object-cover shadow-xl shadow-slate-100" src="<?= base_url('../images/Image1.png') ?>" alt="Smart job recommendations">
    </div>
</section>

<div class="mt-6 flex flex-wrap gap-3">
    <a href="<?= base_url('job') ?>" class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100">All Jobs</a>
...
<?php if (empty($recommendedJobs)): ?>
    <div class="mt-6 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">No recommendation available yet. Add a resume or apply to more jobs to improve matching.</div>
<?php else: ?>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100 bg-white/70">
                    <?php foreach ($recommendedJobs as $job): ?>
                        <tr class="align-top">
                            <td class="px-6 py-5 font-semibold text-slate-900"><?= htmlspecialchars($job['title'], ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="px-6 py-5">
                            ...
                            <td class="px-6 py-5 text-slate-600"><?= htmlspecialchars($job['location'], ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="px-6 py-5 text-slate-600"><?= htmlspecialchars($job['salary'], ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="px-6 py-5 text-slate-600"><?= number_format(((float)$job['recommendation_score']) * 100, 1) ?>%</td>
                            <td class="px-6 py-5">
