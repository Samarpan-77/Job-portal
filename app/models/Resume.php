...
class Resume
{
    ...
    public static function create($user_id, $data, $templateId = 'classic')
    {
        $db = self::db();
        ...
        $stmt = $db->prepare("
            INSERT INTO resumes (user_id,content_json,template_id)
            VALUES (?,?,?)
        ");
        return $stmt->execute([$user_id, $jsonData, $templateId]);
    }
    ...
    public static function updateTemplate($id, $user_id, $templateId)
    {
        $db = self::db();
        $templateId = (string)$templateId ?: 'classic';
        ...
        $stmt = $db->prepare("
            ...
            WHERE id = ? AND user_id = ?
        ");
        return $stmt->execute([$templateId, $id, $user_id]);
    }
    ...
    public static function parseLinkedInText($text)
    {
        ...
        if (!empty($headerLines)) {
            ...
            if (isset($headerLines[2])) {
                $result['address'] = trim($headerLines[2]);
            }
        }
